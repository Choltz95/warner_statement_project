...
do
    f_name=${f::-4}
    if [ "$f" == "custom_dict_large.txt" ] || [ "$f" == "error_log.txt" ] || [ "$f" == "custom_dict.txt" ] || [ "$f" == "sp_test.txt" ]; then
        continue;
    fi
...
#    rm "cleaned_files/${f_name}_processed.txt"
    echo "counting spelling/grammar errors..."
    python count_errors.py "cleaned_files/${f_name}_processed.txt" >> "error_log.txt"
